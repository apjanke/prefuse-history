...
*/
public class LargeSparseDoubleMatrix2D extends DoubleMatrix2D {
	/*
 ...
 */
public LargeSparseDoubleMatrix2D(double[][] values) {
 ...
 */
public LargeSparseDoubleMatrix2D(int rows, int columns) {
 ...
 * @throws	IllegalArgumentException if <tt>rows<0 || columns<0 || (double)columns*rows > Integer.MAX_VALUE</tt>.
 */
public LargeSparseDoubleMatrix2D(int rows, int columns, int initialCapacity, double minLoadFactor, double maxLoadFactor) {
	setUp(rows,columns); 
 ...
 */
protected LargeSparseDoubleMatrix2D(int rows, int columns, AbstractLongDoubleMap elements, int rowZero, int columnZero, int rowStride, int columnStride) {
...
public DoubleMatrix2D assign(DoubleMatrix2D source) {
	// overriden for performance only
	if (! (source instanceof LargeSparseDoubleMatrix2D)) {
		return super.assign(source);
	}
	LargeSparseDoubleMatrix2D other = (LargeSparseDoubleMatrix2D) source;
...
public DoubleMatrix2D assign(final DoubleMatrix2D y, cern.colt.function.DoubleDoubleFunction function) {
	...
	if (function==cern.jet.math.Functions.mult) { // x[i] = x[i] * y[i]
		this.elements.forEachPair(
			new cern.colt.function.LongDoubleProcedure() {
				public boolean apply(long key, double value) {
					long i = key/columns;
					long j = key%columns;
					double r = value * y.getQuick((int)i,(int)j);
		...
		this.elements.forEachPair(
			new cern.colt.function.LongDoubleProcedure() {
				public boolean apply(long key, double value) {
					long i = key/columns;
					long j = key%columns;
					double r = function.apply((int)i,(int)j,value);
					if (r!=value) elements.put(key,r);
					return true;
				}
			}
...
protected boolean haveSharedCellsRaw(DoubleMatrix2D other) {
	if (other instanceof LargeSparseDoubleMatrix2D) {
		LargeSparseDoubleMatrix2D otherMatrix = (LargeSparseDoubleMatrix2D) other;
...
/**
 * Returns the index as a long. The normal index method (maintained for
 *  super-class compatibility) may suffer from integer overflow.
 * @param row
 * @param column
 * @return
 */
protected long trueIndex(int row, int column) {
    //  return super.index(row,column);
	// manually inlined for speed:
	return rowZero + ((long)row)*rowStride 
			+ columnZero + ((long)column)*columnStride;
}
/**
...
public DoubleMatrix2D like(int rows, int columns) {
	return new LargeSparseDoubleMatrix2D(rows, columns);
}
/**
...
public void setQuick(int row, int column, double value) {
	...
	//manually inlined:
	long index = rowZero + ((long)row)*rowStride 
					+ columnZero + ((long)column)*columnStride;
